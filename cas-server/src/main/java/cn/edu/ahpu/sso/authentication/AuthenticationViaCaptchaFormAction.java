...
import cn.edu.ahpu.sso.model.UsernamePasswordCaptchaCredential;
...
public class AuthenticationViaCaptchaFormAction extends AuthenticationViaFormAction {
	...
	public final String validateCaptcha(final RequestContext context,  Credential credential,
		...
		System.out.println("获取Session验证码:" + rand +" <--> "+"获取表单输入验证码:" + captcha);
		...
		// 这段网上这么写的messageContext.addMessage(new
		// MessageBuilder().code("required.captcha").build());
